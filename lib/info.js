...
exports.getBasicInfo = async(id, options) => {
  ...
  const jsonStr = util.between(body, 'ytplayer.config = ', '</script>') || util.between(body, 'ytInitialPlayerResponse = ', '</script>');
  let config;
  // get html5 base.js if there's not any
  let baseJs = body.match(/("jsUrl":")+?[\s\S]+?"/g)[0];
  if (baseJs) {
    baseJs = baseJs.replace(/"/g, '').split(':')[1];
    if (baseJs) {
      additional.html5player = baseJs;
    } 
  }
  if (jsonStr) {
...
const gotConfig = async(id, options, additional, config, fromEmbed) => {
  ...
  const player_response = (config.args && config.args.player_response) || (info && info.player_response);
  ...
  if (info.status === 'fail') {
    ...
    try {
      ...
      if (!info.player_response.formats) {
        info.player_response.streamingData = config.streamingData;
      }
    } catch (err) {
  ...
  if (playability && playability.status === 'UNPLAYABLE' && !(info.player_response.formats || info.player_response.streamingData.adaptiveFormats)) {
  ...
  info.html5player = config.assets ? config.assets.js : additional.html5player;
...
exports.getFullInfo = async(id, options) => {
  ...
  if (info.html5player) {
    ...
    //there're some problems with this one, will be fixed later
