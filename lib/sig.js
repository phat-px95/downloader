...
exports.setDownloadURL = (format, sig, debug) => {
  ...
  try {
    decodedUrl = decodeURIComponent(format.signatureCipher.match(/url=[\s\S]+/g)[0]?.split('=')[1]) + '&sig=' + sig;
  } catch (err) {
    if (debug) {
  ...
  format.url = url.format(decodedUrl);
};
...
exports.decipherFormats = (formats, tokens, debug) => {
  formats.forEach(format => {
    ...
    // in cases there're no url --> decode signatureCipher
    if (!format.s && !format.url && format.signatureCipher) {
      format.signatureCipher = format.signatureCipher.split('&')[0].split('=')[1];
    }
    const sig = tokens && format.signatureCipher ? exports.decipher(tokens, decodeURIComponent(format.signatureCipher)) : null;
    exports.setDownloadURL(format, sig, debug);
  });
};
